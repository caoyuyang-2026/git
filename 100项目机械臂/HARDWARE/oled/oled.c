...
/***********功能描述：显示BMP图片，起始点坐标(x,y),图片宽度width，高度height*****************/
void oled_draw_picture(unsigned char x0, unsigned char y0, unsigned char width, unsigned char height, const unsigned char BMP[])
{ 	
 unsigned int j=0;
 unsigned char x,y;
 unsigned char y1;
...
//显示进度条
//x,y: 进度条左上角坐标（x为像素，y为页号）
//width: 进度条宽度（像素）
//height: 进度条高度（页）
//progress: 进度值（0-100）
void OLED_ShowProgressBar(unsigned char x, unsigned char y, unsigned char width, unsigned char height, unsigned char progress)
{
	unsigned char i, j;
	unsigned char fill_width;
	
	// 计算填充宽度
	fill_width = (progress * width) / 100;
	...
	for(i = 1; i < height - 1; i++)
	{
		...
		{
			if(j <= fill_width)
				OLED_WR_Byte(0xFF, OLED_DATA);  // 填充
			else
				OLED_WR_Byte(0x00, OLED_DATA);  // 空白
		}
	}
}
